...
namespace LINQPractice
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var titlesForProductsWithPricesInRange10To20OrderedByTitleAscFunctional = products.Where(product => product.Price >= 10.00 && product.Price <= 20.00).Select(product => product.Title).OrderBy(title => title);
            ...
            var idsForProductsWithPricesInRange10To20OrderedByIDDescFunctional = products.Where(product => product.Price >= 10.00 && product.Price <= 20.00).Select(product => product.ID).OrderByDescending(product => product);
             Console.WriteLine("Non-functional:");
            ...
            var computerProductsCostingMoreThan100 = from product in products where product.Category == ProductCategory.Computers && product.Price > 100 select product;
            var computerProductsCostingMoreThan100Functional = products.Where(product => product.Category == ProductCategory.Computers && product.Price > 100);
            ...
            var productTitleWithID3152 = (from product in products where product.ID == 3152 select product.Title).FirstOrDefault();
            var productTitleWithID3152Functional = products.FirstOrDefault(product => product.ID == 3152);
            ...
            var petProductsLowestToHighestPrice = from product in products where product.Category == ProductCategory.Pet orderby product.Price select product;
